...
public class AppCenter.Views.AppInfoView : Adw.NavigationPage {
    ...
    construct {
        ...
        var addons = package_component.get_addons ();
        if (addons.length > 0) {
            var extensions = new ListStore (typeof (AppCenterCore.Package));

            foreach (var addon in addons) {
                var extension_package = AppCenterCore.FlatpakBackend.get_default ().get_package_for_component_id (addon.id);
                if (extension_package == null) {
                    return;
                }

                extensions.append (extension_package);
            }

            var extension_view = new Widgets.PackageGridView (extensions);

            extension_view.package_activated.connect ((pkg) => show_other_package (pkg));
            ...
            content_box.append (extension_view);
        }
