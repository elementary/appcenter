...
namespace AppCenter.Views {
    public class UpdateView : Gtk.ScrolledWindow {
        ...
        public UpdateView () {
            ...
            Client.get_default ().operation_changed.connect ((operation, running) => {
                if (operation == Operation.UPDATE_REFRESH && !running)
                    update_ui (Client.get_default ().update_list);
            });

            Client.get_default ().refresh_updates ();
            update_ui (Client.get_default ().update_list);
        }

        private void build_ui () {
            Gtk.Label head_label = new Gtk.Label
                //(@"<span font_weight=\"bold\" size=\"x-large\">%s</span>".printf (_("Software Updates")));
                (_("Software Updates"));
        ...
        private Gtk.ListBoxRow create_row (Summary summary) {
            ...
            Gtk.Label label = new Gtk.Label
                 (@"<span font_weight=\"bold\" size=\"x-large\">%s</span>".printf (summary.display_name));
