...
public class AppCenterCore.PkPackage : Package, Object {
    ...
    public AppStream.Component component { public get; construct; }
    ...
    public Package.State state { public get; private set; }
    ...
    public bool update_available {
        get {
            return state == Package.State.UPDATE_AVAILABLE;
        }
    }
    ...
    public bool is_updating {
        get {
            return state == Package.State.UPDATING;
        }
    }
    ...
    public PkPackage (AppStream.Component component) {
    ...
    public void update_state () {
        if (installed) {
            if (change_information.has_changes ()) {
                state = Package.State.UPDATE_AVAILABLE;
            } else {
                state = Package.State.INSTALLED;
            }
        } else {
            state = Package.State.NOT_INSTALLED;
        }
    }

    public async bool update () {
        if (state != Package.State.UPDATE_AVAILABLE) {
            return false;
        }

        try {
            return yield perform_operation (Package.State.UPDATING, State.INSTALLED, State.UPDATE_AVAILABLE);
        } catch (Error e) {
            return false;
        }
    }
    ...
    public async bool install () {
        if (state != Package.State.NOT_INSTALLED) {
            return false;
        }

        try {
            bool success = yield perform_operation (Package.State.INSTALLING, State.INSTALLED, State.NOT_INSTALLED);
            if (success) {
                var backend = AppCenterCore.PkBackend.get_default ();
                backend.operation_finished (this, Package.State.INSTALLING, null);
            }

            return success;
        } catch (Error e) {
            var backend = AppCenterCore.PkBackend.get_default ();
            backend.operation_finished (this, Package.State.INSTALLING, e);
            return false;
        }
    }

    public async bool uninstall () {
        if (state == Package.State.INSTALLED || state == State.UPDATE_AVAILABLE) {
            try {
                return yield perform_operation (Package.State.REMOVING, State.NOT_INSTALLED, state);
            } catch (Error e) {
                return false;
            }
        }
    ...
    private async bool perform_operation (Package.State performing, Package.State after_success, Package.State after_fail) throws GLib.Error {
    ...
    private void prepare_package_operation (Package.State initial_state) {
        changing (true);
    ...
    private async Pk.Exit perform_package_operation () throws GLib.Error {
        ...
        switch (state) {
            case Package.State.UPDATING:
                return yield backend.update_package (this, cb, action_cancellable);
            case Package.State.INSTALLING:
                return yield backend.install_package (this, cb, action_cancellable);
            case Package.State.REMOVING:
                return yield backend.remove_package (this, cb, action_cancellable);
            default:
                return Pk.Exit.UNKNOWN;
        }
    }

    private void clean_up_package_operation (Pk.Exit exit_status, Package.State success_state, Package.State fail_state) {
        changing (false);
    ...
    public Pk.Package? find_package () {
        ...
        try {
            pk_package = AppCenterCore.PkBackend.get_default ().get_app_package (component.get_pkgnames ()[0], 0);
        } catch (Error e) {
            warning (e.message);
            return null;
        }
